...
Modified to remove axis approximation, deadzones and provide compatability with Guitar Hero controllers
...
def struct_dict(struct):
    ...
    >>> 'buttons' in struct_dict(XInputGamepad)
    True
    >>> struct_dict(XInputGamepad)['buttons'].__class__.__name__
    'CField'
    """
...
class XInputJoystick(event.EventDispatcher):
    ...
    def get_battery_information(self):
        ...
        XInputGetBatteryInformation.argtypes = [ctypes.c_uint, ctypes.c_ubyte, ctypes.POINTER(XInputBatteryInformation)]
        XInputGetBatteryInformation.restype = ctypes.c_uint

        battery = XInputBatteryInformation(0,0)
